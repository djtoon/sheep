...
for i in $(seq 1 $N); do
  X=$((5040 + (i * 37) % 120)); Q="stage=${STAGE:-1}&x=$X&weapon=$W"; [ "$G" = 1 ] && Q="god=1&$Q"
  ...
  node art/raw/feel/play.mjs --query "$Q" --bot 1 --max 180000 --eval "$SEED" --until "$U" \
    --print "(()=>{const s=__sheep.game.scene.getScene('Game'); const b=s.boss; const dl=s.stat?s.stat.deathLog:[]; const f=v=>v==null?null:+(v/1000).toFixed(1); return {w:'$W',god:$G,run:$i,x:$X,p1:b&&b.transAt?f(b.transAt-b.p1At):null,p2:b&&b.diedAt&&b.transAt?f(b.diedAt-b.transAt):null,cleared:!!s.cleared,d1:dl.filter(d=>b&&b.p1At&&d.t>=b.p1At&&(!b.transAt||d.t<b.transAt)).length,d2:dl.filter(d=>b&&b.transAt&&d.t>=b.transAt).length,dAll:dl.length,coreHp:b?(b.core||b.organ).hp:null}})()" 2>&1 | grep -E "^print|pageerror" &
done; wait
